  ...
  Column(modifier = Modifier.fillMaxSize(),
      ...
      if (data.isNotEmpty()) {
          ...
          LazyColumn(
              ...
              item {
                  TextUserAgreementContent(doc = document)
              }
          }
      } else {
...
@Composable
fun TextUserAgreementContent(doc: Document){


    // Отображение содержания мелким шрифтом
    AndroidView(
        factory = { context ->
...
@Composable
fun UserAgreementScreenPreview(){
    UserAgreementScreen(navController = NavHostController(LocalContext.current))
}
